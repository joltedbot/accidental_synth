...
fn log_ui_upgrade_failure(function_name: &str, result: Result<(), slint::EventLoopError>) {
    if let Err(e) = result {
        log::warn!(target: "ui::set_slint_values", "{function_name}(): Failed to upgrade UI weak reference: {e}");
    }
}
...
pub fn set_midi_screen_values(
    ...
    log_ui_upgrade_failure(
        "set_midi_screen_values",
        ui_weak_thread.upgrade_in_event_loop(move |ui| {
...
pub fn set_lfo_values(
    ...
    log_ui_upgrade_failure(
        "set_lfo_values",
        ui_weak_thread.upgrade_in_event_loop(move |ui| match lfo_index {
            LFOIndex::ModWheel => {
...
fn set_ui_envelope_values(
    ...
    log_ui_upgrade_failure(
        "set_ui_envelope_values",
        ui_weak_thread.upgrade_in_event_loop(move |ui| match envelope_index {
            EnvelopeIndex::Amp => {
                ui.set_amp_envelope_values(ui::slint_envelope_from_ui_envelope(
                    &ui_envelope_values,
                ));
            }
            EnvelopeIndex::Filter => {
                ui.set_filter_envelope_values(ui::slint_envelope_from_ui_envelope(
                    &ui_envelope_values,
                ));
            }
            EnvelopeIndex::Pitch => {
                ui.set_pitch_envelope_values(ui::slint_envelope_from_ui_envelope(
                    &ui_envelope_values,
                ));
            }
        }),
...
pub fn set_oscillator_mixer_values(
    ...
    log_ui_upgrade_failure(
        "set_oscillator_mixer_values",
        ui_weak_thread.upgrade_in_event_loop(move |ui| {
            ui.set_oscillator_mixer_values(ModelRc::from(Rc::new(VecModel::from(
                oscillator_mixers,
            ))));
        }),
    );
