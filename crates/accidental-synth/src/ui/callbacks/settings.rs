...
pub fn callback_midi_input_channel_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_midi_input_channel_changed(move |channel| {
            log::trace!(target: "ui::settings", "callback_midi_input_channel_changed(): Sending MidiDeviceUpdateEvents::UIMidiInputChannelIndex : {channel}");
...
pub fn callback_midi_input_port_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_midi_input_port_changed(move |port|{
            log::trace!(target: "ui::settings", "callback_midi_input_port_changed(): Sending MidiDeviceUpdateEvents::UIMidiInputPort : {port}");
...
pub fn callback_audio_output_device_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_audio_output_device_changed(move |device| {
            log::trace!(target: "ui::settings", "callback_audio_output_device_changed(): Sending AudioDeviceUpdateEvents::UIOutputDevice : {device}");
...
pub fn callback_audio_output_left_channel_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_audio_output_left_channel_changed(move |channel| {
            ...
            log::trace!(target: "ui::settings", "callback_audio_output_left_channel_changed(): Sending AudioDeviceUpdateEvents::UIOutputDeviceLeftChannel : {channel_number}");
...
pub fn callback_audio_output_right_channel_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_audio_output_right_channel_changed(move |channel| {
            let channel_number = channel.parse::<i32>().unwrap_or(AUDIO_DEVICE_CHANNEL_NULL_VALUE);
            log::trace!(target: "ui::settings", "callback_audio_output_right_channel_changed(): Sending AudioDeviceUpdateEvents::UIOutputDeviceRightChannel : {channel_number}");
...
pub fn callback_audio_sample_rate_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_audio_sample_rate_changed(move |rate| {
            log::trace!(target: "ui::settings", "callback_audio_sample_rate_changed(): Sending AudioDeviceUpdateEvents::SampleRateChanged : {rate}");
...
pub fn callback_audio_buffer_size_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_audio_buffer_size_changed(move |size| {
            log::trace!(target: "ui::settings", "callback_audio_buffer_size_changed(): Sending AudioDeviceUpdateEvents::BufferSizeChanged : {size}");
            audio_output_device_sender
...
pub fn callback_patch_changed(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_patch_changed(move |patch_index| {
            log::trace!(target: "ui::settings", "callback_patch_changed(): Sending SynthesizerUpdateEvents::PatchChanged : {patch_index}");
            ...
            log::trace!(target: "ui::settings", "callback_patch_changed(): Sending UIUpdates::Patches : {patch_index}");
...
pub fn callback_patch_saved(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_patch_saved(move |patch_name| {
            log::trace!(target: "ui::settings", "callback_patch_saved(): Sending SynthesizerUpdateEvents::PatchSaved : {patch_name}");
...
pub fn callback_patch_deleted(
    ...
    if let Some(ui) = ui_weak.upgrade() {
        ui.on_patch_deleted(move |patch_name| {
            log::trace!(target: "ui::settings", "callback_patch_deleted(): Sending SynthesizerUpdateEvents::PatchDeleted : {patch_name}");
