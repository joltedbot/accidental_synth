...
| `synced_frequency` | number | | [Performance state](#performance-state-fields) — computed at runtime from clock and `thirty_second_notes`; leave as 0.5 |
| `sync_triggered` | boolean | | [Performance state](#performance-state-fields) — set by the engine at each 32nd-note clock boundary; leave as false |
| `gate_flag` | boolean | | [Performance state](#performance-state-fields) — leave as false |
