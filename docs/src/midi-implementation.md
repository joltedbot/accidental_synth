...
8. **LFO Control**: Two LFOs are available - one controlled by the mod wheel (CC #1) with parameters on CCs 100-107, and one for filter modulation with parameters on CCs 108-114.

9. **Effects**: Built-in effects include Saturation, Colour Compressor, Wave Folder, Bit Shifter, Clipper, Gate Clipping, Wave Rectifier, Chorus, Auto-Pan, Tremolo, and Delay. Some effects parameters can be controlled via aftertouch and CCs.
