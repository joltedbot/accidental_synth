...
System presets are located in `crates/accsyn-engine/src/synthesizer/patches/` and use the `.json` file extension. `init.json` is loaded by default 
at system startup.
...
| Field        | Type       | Range   | Description                                 |
|--------------|------------|---------|---------------------------------------------|
| `name`       | string     |         | Effect name (informational; used by the UI) |
